...
export default function createInitRouter() {
  ...
  router.get("/dashboard", async (req, res) => {
    ...
    // ✅ ÉTAPE 2: Récupérer depuis la DB pour le round actuel (si round existe)
    if (currentRoundId) {
      try {
        // Convertir roundId en nombre pour la requête DB
        const roundIdForDb = typeof currentRoundId === 'string' ? parseInt(currentRoundId, 10) : currentRoundId;
        ...
        const dbReceiptsResult = await pool.query(
          `SELECT r.*, 
           ...
           WHERE r.round_id = $1 AND r.status != 'cancelled'
           GROUP BY r.receipt_id 
           ORDER BY r.created_at DESC`,
          [roundIdForDb]
        );
        
        console.debug(`[DASHBOARD-API] ${dbReceiptsResult.rows.length} ticket(s) trouvé(s) en DB pour round ${currentRoundId} (cancelled exclus)`);
